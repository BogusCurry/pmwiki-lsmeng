<?php
/* 
 * This is a modified version of PmWiki written and maintained by Ling-San Meng. The
 * current version as it stands is mainly meant for personal use, such as a personal web
 * blog or research journal. The single most notable change to the PmWiki core is the
 * support of whole page encryption/decryption. Upon the first use, the password entered
 * at the password prompt will be used for future page encryption/decryption. For best
 * security, the password is not stored in any form in the configuration file. 
 * Password check is done by performing decryption and looking for known plain text
 * generated by PmWiki automatically. Such an approach is chosen since the encryption
 * algorithm chosen is immune to known-plaintext attact, as most modern encryption
 * algorithms are; and most system breaches are not due to defects in encryption
 * algorithms, but due to security holes in storing password hashes. Please refer to the 
 * homepage of PmWiki for basic guides and introduction. Please email me for detailed
 * features and support of each version if you are interested.
 *
...
href='{\$PageUrl}/edit'>\$LinkText</a><a rel='nofollow'
class='createlink' href='{\$PageUrl}/edit'>?</a>";
$UrlLinkFmt =
...
a.createlink
{
  text-decoration:none; position:relative; top:-0.5em;
...
function PHSC($x, $flags=ENT_COMPAT, $enc=null, $dbl_enc=true)
{
  # for PHP 5.4
...
function PCCF($code, $template = 'default', $args = '$m')
{
  ...
  {
    // Meng. Fix for the unset match offset; specifically offset 2/3/4 cause problems
    $code = preg_replace('/\$m\[([234])\]/', '(isset($m[$1])?$m[$1]:null)', $code);
...
class PageStore
{
  ...
  function read($pagename, $since=0)
  {
    ...
    if (preg_match("/Site[\.\/]EditQuickReference/i", $pagename)) { return; }
...
function Redirect($pagename, $urlfmt='$PageUrl')
{
  ...
  if ($EnablePathInfo === 1) { $pagename = preg_replace("/\./", "/", $pagename); }
...
*/
    ...
    // Should be clear.
    else if (preg_match("/Main[\.\/]Phpinfo/i", $pagename)) { phpinfo(); return; }
    ...
    else if (preg_match("/Main[\.\/]Map/i", $pagename)) {}

    // DEBUG pageindex
    else if (preg_match("/Main[\.\/]PageindexLog/i", $pagename))
    {
      global $pageindexTimeDir;
      $text = "[@".file_get_contents("$pageindexTimeDir/log.txt")."@]";
    }

    else if (preg_match("/Main[\.\/]SysLog/i", $pagename))
    {
      global $sysLogFile;
...
function PmWikiAuth($pagename, $level, $authprompt=true, $since=0)
{
  ...
  if (($level=='ALWAYS' || @$page['=auth'][$level]) && isset($_SESSION['MASTER_KEY']))
  {
    ...
    {
      // For clicking the logout button, or triggering by pagetimer
  ...
  if (substr($pagename,0,strlen("LOGOUT")) != "LOGOUT" && !preg_match("/Main[\.\/]ChangePassword/i", $pagename))
  {
    global $actionStr;
