...
*/
					...
					// For all the direct children of the document body
					for (var i=0;i<bodyElementLen;i++)
					{
						// if it's wikibody
						var docChild = documentBodyElement[i];
						if (docChild.id == 'wikibody')
						{
							// For all the direct children
							for (var j=0;j<docChild.children.length;j++)
							{
								var wikibodyChild = docChild.children[j];
								
								// if it's wikitext
								if (wikibodyChild.id == 'wikitext')
								{
									// for all the direct children 
									for (var k=0;k<wikibodyChild.children.length;k++)
									{
										var wikitextChild = wikibodyChild.children[k];
										// if it's an UL 
										if (wikitextChild.tagName == 'UL')
										{
											// for all the direct children 
											// blur if it's visible	
											for (var h=0;h<wikitextChild.children.length;h++)
											{
											  if (isElementVisible(wikitextChild.children[h]))
											  { ImgfocusBlurElement(wikitextChild.children[h]); }
											}
										}
										// else blur the element if it's visible
										else
										{
										  if (isElementVisible(wikitextChild))
											{ ImgfocusBlurElement(wikitextChild); }
										}
									}
								}
								else
								{
									// else blur the element if it's visible
									if (isElementVisible(wikibodyChild))
									{ ImgfocusBlurElement(wikibodyChild); }								  
								}
							}
						}
						else
						{
							// else blur the element if it's visible
							if (isElementVisible(docChild))
							{ ImgfocusBlurElement(docChild); }						  
						}
					}
...
function isElementVisible(element)
{
  ...
  var box = element.getBoundingClientRect();
	...
	// see if box.left < the right border && box.right > window left border
	if (box.left < window.innerWidth && box.right > 0)
	{
	  if (box.top < window.innerHeight && box.bottom > 0) { return true; }
	}
...
function ImgfocusBlurElement(element)
{
  ...
  element.style.opacity = 0.8;
	if (!('ImgfocusBlurredElements' in window)) { window.ImgfocusBlurredElements = []; }
  window.ImgfocusBlurredElements.push(element);
}
...
{  
  ...
  if (imgEnlarge != null)
    ...
    document.body.style.overflow = 'auto';		
		...
		var NUM_BLUR = window.ImgfocusBlurredElements.length;
		for (var i=0;i<NUM_BLUR;i++)
		{ ImgfocusUnBlurElement(window.ImgfocusBlurredElements[i]); }
		window.ImgfocusBlurredElements = [];
